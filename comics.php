...
function createComicCard(comic, showProgress = false) {
   ...
   // Sistema de Progresso de Leitura - INTEGRADO COM BANCO + LOCALSTORAGE
async function setupReadingProgress() {
    try {
        ...
        // CARREGAR PROGRESSO DO LOCALSTORAGE (NOVO - do segundo código)
        const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
        
        // Combinar progresso do servidor com localStorage
        allComics.forEach(comic => {
            if (savedProgress[comic.id]) {
                // Priorizar progresso mais recente
                const serverProgress = comic.progress || 0;
                const localProgress = savedProgress[comic.id].progress || 0;
                
                // Usar o progresso mais alto
                comic.progress = Math.max(serverProgress, localProgress);
                comic.currentPage = savedProgress[comic.id].currentPage || 1;
                comic.lastRead = savedProgress[comic.id].lastRead || comic.lastRead;
            }
        });
        ...
        // Ainda carregar do localStorage em caso de erro
        const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
        allComics.forEach(comic => {
            if (savedProgress[comic.id]) {
    ...
    console.log('Quadrinhos do BD:', dbComics);
    console.log('Progressos:', progressData);
    
    const combined = dbComics.map(comic => {
        // ⭐ DEBUG DETALHADO: Verificar se encontra progresso
        const progressInfo = progressData.find(p => {
            const match = p.comic_id === comic.id;
            console.log(`Comparando: progress.comic_id=${p.comic_id} vs comic.id=${comic.id} -> ${match ? '✅' : '❌'}`);
            return match;
        });
        
        console.log(`Quadrinho ${comic.id} ("${comic.title}"):`, 
                   progressInfo ? `Progresso ${progressInfo.progress_pct}%` : 'Sem progresso');
    ...
    // ⭐ DEBUG FINAL
    const withProgress = combined.filter(c => c.progress > 0);
    console.log(`🎯 RESULTADO: ${withProgress.length} de ${combined.length} quadrinhos têm progresso`);
   ...
   function updateContinueReadingSection() {
    ...
    if (continueReadingComics.length === 0) {
        ...
        // ⭐ CORREÇÃO: COLOCAR DENTRO DO ELSE
...
function saveReadingProgress(comicId, progress, currentPage = 1) {
    ...
    if (comicIndex !== -1) {
        ...
        allComics[comicIndex].currentPage = currentPage;
        allComics[comicIndex].lastRead = new Date().toISOString();
    }
    
    // Salvar também no localStorage (NOVO)
    saveReadingProgressLocal(comicId, progress, currentPage);
    ...
    .then(data => {
        if (data.ok) {
            console.log(`Progresso salvo no servidor: Quadrinho ${comicId} - ${progress}%`);
    ...
    .catch(error => {
        ...
        // Mesmo com erro no servidor, atualizar a interface com dados locais
        updateContinueReadingSection();
    });
}
    ...
    // Modal de detalhe do quadrinho
    // Modal de detalhe do quadrinho - ATUALIZADO
// Modal de detalhe do quadrinho - ATUALIZADA COM PROGRESSO LOCAL
function openComicModal(comic) {
    ...
    // Verificar se já tem progresso de leitura (NOVO - do segundo código)
    const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
    const hasProgress = savedProgress[comic.id] && savedProgress[comic.id].progress > 0;
    const serverProgress = comic.progress > 0;
    
    // Atualizar texto do botão baseado no progresso
    if (hasProgress || serverProgress) {
        const progress = hasProgress ? savedProgress[comic.id].progress : comic.progress;
    ...
    readBtn.onclick = function() {
        // Salvar progresso antes de redirecionar (NOVO)
        if (!hasProgress && !serverProgress) {
    ...
    // NOVO: Botão de comentários (mantido do primeiro código)
    ...
    // FUNÇÕES ADICIONADAS DO SEGUNDO CÓDIGO

// Salvar progresso no localStorage (complementar ao servidor)
function saveReadingProgressLocal(comicId, progress, currentPage = 1) {
    const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
    ...
    console.log(`Progresso local salvo: Quadrinho ${comicId} - ${progress}%`);
}

// Obter quadrinhos para "Continuar Lendo" (versão melhorada)
function getContinueReadingComics() {
    const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
    
    return allComics
        .filter(comic => {
            const hasProgress = comic.progress > 0 && comic.progress < 100;
            const hasLocalProgress = savedProgress[comic.id] && savedProgress[comic.id].progress > 0 && savedProgress[comic.id].progress < 100;
            return hasProgress || hasLocalProgress;
        })
        .sort((a, b) => {
            const progressA = savedProgress[a.id] || { lastRead: a.lastRead || '0' };
            const progressB = savedProgress[b.id] || { lastRead: b.lastRead || '0' };
            return new Date(progressB.lastRead) - new Date(progressA.lastRead);
        });
}

// Atualizar info do quadrinho em progresso (versão melhorada)
function updateContinueInfo(comic) {
    const continueInfo = document.getElementById('continueInfo');
    const infoTitle = document.getElementById('infoTitle');
    const infoMeta = document.getElementById('infoMeta');
    ...
    if (comic && (comic.progress > 0 || comic.currentPage > 1)) {
        const savedProgress = JSON.parse(localStorage.getItem('hq-verso-reading-progress')) || {};
        const localProgress = savedProgress[comic.id];
        
        const progress = localProgress ? localProgress.progress : comic.progress;
        const currentPage = localProgress ? localProgress.currentPage : (comic.currentPage || 1);
        
        infoTitle.textContent = comic.title;
        infoMeta.textContent = `${progress}% lido • Continuar da página ${currentPage}`;
        infoProgress.style.width = `${progress}%`;
        continueInfo.style.display = 'block';
    } else {
        continueInfo.style.display = 'none';
    }
...
// Simular progresso no leitor (para testes)
function simulateReadingProgress(comicId, pagesRead, totalPages = 100) {
    ...
    saveReadingProgressLocal(comicId, progress, pagesRead); // Salvar local também
    updateContinueReadingSection();
}
    
    // Event Listener para progresso do leitor (NOVO)
...
// Simular clique nos badges "Ler agora" com progresso
document.addEventListener('click', function(e) {
    if (e.target.closest('.read-badge')) {
        const card = e.target.closest('.comic-card');
        if (card) {
            const comicId = parseInt(card.getAttribute('data-id'), 10);
            const comic = allComics.find(c => c.id === comicId);
            ...
            if (comic) {
                ...
                if (!hasProgress && comic.progress === 0) {
                    // Se for novo, salvar progresso inicial
                    saveReadingProgress(comicId, 10, 1);
                }
            }
        }
    }
